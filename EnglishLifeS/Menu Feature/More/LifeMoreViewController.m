...
#import "LifeMoreViewController.h"
#import "UIImageView+WebCache.h"
#import "FileSize.h"
#import "LifeMoreAppCell.h"

#import "LifemBookmarkViewController.h"
#import "MBProgressHUD.h"
#import <MessageUI/MFMailComposeViewController.h>
#import <StoreKit/StoreKit.h>

@interface LifeMoreViewController ()<MFMailComposeViewControllerDelegate,SKStoreProductViewControllerDelegate>
{
    NSArray *gheikcmaeoxqresultArray;
    NSMutableArray *gheikcmaeoxqappArray;
}
@end

@implementation LifeMoreViewController
...
- (void)viewDidLoad
{
    ...
    gheikcmaeoxqappArray = [[NSMutableArray alloc] initWithCapacity:10];
    
    
    gheikcmaeoxqresultArray = [[NSArray alloc] initWithObjects:
...
//                    [NSArray arrayWithObjects:@"意见建议反馈",@"plugin_icon_message",nil],
    ...
    if (gheikcmaeoxqappArray.count == 0) {
        [self loadData];
    }
...
{
    return gheikcmaeoxqappArray.count<2?3:4;
}
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
	return  section == 3 ? gheikcmaeoxqappArray.count : [[gheikcmaeoxqresultArray objectAtIndex:section] count];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section < 3) {
        ...
        NSArray *data = [[gheikcmaeoxqresultArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
        ...
        LifeMoreAppCell *cell = (LifeMoreAppCell *)[tableView dequeueReusableCellWithIdentifier:MyIdentifier]; // changed this
        if (cell == nil) {
            cell = [[LifeMoreAppCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:MyIdentifier]; // changed this
            cell.accessoryType = UITableViewCellAccessoryNone;
            //cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        
        NSDictionary *app = [gheikcmaeoxqappArray objectAtIndex:indexPath.row];
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section{
    if (section==3 && gheikcmaeoxqappArray.count==0) {
        return 40;
    }
    return 0;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0){
            LifemBookmarkViewController *bk = [[LifemBookmarkViewController alloc] init];
...
//        }
        ...
        NSLog([[gheikcmaeoxqappArray objectAtIndex:indexPath.row] objectForKey:@"app_id"]);
        [self openAppWithId:[[gheikcmaeoxqappArray objectAtIndex:indexPath.row] objectForKey:@"app_id"]];
    }
}
...
-(void)loadData{
    ...
    [request setCompletionBlock:^{
        ...
        gheikcmaeoxqappArray = [NSJSONSerialization JSONObjectWithData:responseData options:NSJSONReadingMutableLeaves error:nil];
        //NSLog(@"%@",appArray);
        [self.tableView reloadData];
