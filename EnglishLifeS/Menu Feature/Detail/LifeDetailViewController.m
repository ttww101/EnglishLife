...
@interface LifeDetailViewController ()<UIWebViewDelegate,UIGestureRecognizerDelegate,UIActionSheetDelegate, GADInterstitialDelegate> {
    NSString *gheikcmaeoxqindex;
    UIWebView *gheikcmaeoxqwebView;
    NSUInteger gheikcmaeoxq_currentFontSize;
    UIActivityIndicatorView *gheikcmaeoxqactivityView;
    KLCPopup *gheikcmaeoxqpopupAd;
    NSString *gheikcmaeoxqgotoUrl;
    
    GADInterstitial *gheikcmaeoxq_interstitial;
}
...
@implementation LifeDetailViewController
...
- (void)gotoAdView:(id)sender {
    [gheikcmaeoxqpopupAd dismissPresentingPopup];
    
    // 统计数据
    [TalkingData trackEvent:@"Popup_AD_Touch" label:gheikcmaeoxqgotoUrl];
    [self uploadIDFAData:gheikcmaeoxqgotoUrl];
    
    if([gheikcmaeoxqgotoUrl hasPrefix:@"https://ituns"]){
        // 跳转到itunes
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[gheikcmaeoxqgotoUrl stringByReplacingOccurrencesOfString:@"https" withString:@"itms-apps"]]];
    }else{
        // 跳转到网页
        TOWebViewController *dd = [[TOWebViewController alloc] initWithURLString:gheikcmaeoxqgotoUrl];
...
- (void)viewDidLoad {
    ...
    {
        gheikcmaeoxq_currentFontSize = 100;
    }
    else
    {
        gheikcmaeoxq_currentFontSize = [[NSUserDefaults standardUserDefaults] integerForKey:@"ftsz"];
    }
    

    gheikcmaeoxqwebView = [[UIWebView alloc] initWithFrame:CGRectMake(0.0f,0.0f, self.view.frame.size.width, self.view.frame.size.height)];
    gheikcmaeoxqwebView.autoresizingMask = UIViewAutoresizingFlexibleHeight + UIViewAutoresizingFlexibleWidth;
	[gheikcmaeoxqwebView setUserInteractionEnabled:YES];
	[gheikcmaeoxqwebView setBackgroundColor:[UIColor clearColor]];
	[gheikcmaeoxqwebView setDelegate:self];
	[gheikcmaeoxqwebView setOpaque:NO];
    gheikcmaeoxqwebView.dataDetectorTypes=0;

    NSURL *url=[NSURL URLWithString:[NSString stringWithFormat:@"%@?p=%@",SHOW_URL,gheikcmaeoxqindex]];
    NSURLRequest *request=[[NSURLRequest alloc] initWithURL:url];
    [gheikcmaeoxqwebView loadRequest:request];
    
    
    for (UIView* subView in [gheikcmaeoxqwebView subviews]) {
    ...
    [gheikcmaeoxqwebView addGestureRecognizer:pin];
    ...
    [gheikcmaeoxqwebView addGestureRecognizer:imageTapDetector];
    ...
    [self.view addSubview:gheikcmaeoxqwebView];
    
    //google广告
    AppDelegate *appDelegate = (AppDelegate*)[[UIApplication sharedApplication]delegate];
    if (appDelegate.gheikcmaeoxqgoogleAdSwitch)
    {
        if (gheikcmaeoxq_interstitial) {
            gheikcmaeoxq_interstitial.delegate = nil;
            gheikcmaeoxq_interstitial = nil;
        }
        //interstitial
        gheikcmaeoxq_interstitial = [[GADInterstitial alloc] initWithAdUnitID:kAdMobInterstitialKey];
        gheikcmaeoxq_interstitial.delegate = self;
        
        int i = arc4random() % 3;
        
        if (i == 0) {
            GADRequest *request = [GADRequest request];
            [gheikcmaeoxq_interstitial loadRequest:request];
        }
...
- (void)setADPopup:(NSData *) rawJSONData{
    ...
    gheikcmaeoxqgotoUrl = [[[ret objectForKey:@"list"] objectAtIndex:0] objectForKey:@"ad_url"];
    ...
    gheikcmaeoxqpopupAd = [KLCPopup popupWithContentView:contentView];
    [gheikcmaeoxqpopupAd show];
}
...
- (void)tapDetected:(UITapGestureRecognizer *)tap {
    if (tap.state == UIGestureRecognizerStateRecognized) {
        ...
        NSString *imageURL = [gheikcmaeoxqwebView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"document.elementFromPoint(%f, %f).src", touchPoint.x, touchPoint.y]];
...
- (void)changeFontSize:(FontSizeChangeType)changeType {
    if (changeType == FontSizeChangeTypeIncrease && gheikcmaeoxq_currentFontSize == 160) return;
    if (changeType == FontSizeChangeTypeDecrease && gheikcmaeoxq_currentFontSize == 50) return;
    if (changeType != FontSizeChangeTypeNone)
    {
        gheikcmaeoxq_currentFontSize = (changeType == FontSizeChangeTypeIncrease) ? gheikcmaeoxq_currentFontSize + 5 : gheikcmaeoxq_currentFontSize - 5;
        [[NSUserDefaults standardUserDefaults] setInteger:gheikcmaeoxq_currentFontSize forKey:@"ftsz"];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
    NSString *jsString = [[NSString alloc] initWithFormat:@"document.getElementsByTagName('body')[0].style.webkitTextSizeAdjust= '%lu%%'",
                          (unsigned long)gheikcmaeoxq_currentFontSize];
    [gheikcmaeoxqwebView stringByEvaluatingJavaScriptFromString:jsString];
}


- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation {
    gheikcmaeoxqwebView.frame = CGRectMake(0.0f,0.0f, self.view.frame.size.width, self.view.frame.size.height);
    [gheikcmaeoxqwebView reload];
}
...
-(void)deleteBookmark:(NSString *)__bookmarkid {
    ...
    [gheikcmaeoxqwebView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"document.getElementById('bookmark').innerHTML = '收藏';document.getElementById('bookmark').href = 'bookmark://%@';",__bookmarkid]];
...
-(void)addBookmark:(NSString *)__bookmarkid {
    ...
    if ([favs objectForKey:__bookmarkid]) {
        ...
        NSString *newstitle = [gheikcmaeoxqwebView stringByEvaluatingJavaScriptFromString:@"document.title"];
    ...
    [gheikcmaeoxqwebView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"document.getElementById('bookmark').innerHTML = '已收藏';document.getElementById('bookmark').href = 'deletebookmark://%@';",__bookmarkid]];
...
- (void)webViewDidStartLoad:(UIWebView *)awebView {
    ...
    gheikcmaeoxqactivityView = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
    gheikcmaeoxqactivityView.frame = CGRectMake(0, 0, 24, 24);
    
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:gheikcmaeoxqactivityView];
    [gheikcmaeoxqactivityView startAnimating];
    
    AppDelegate *appDelegate = (AppDelegate*)[[UIApplication sharedApplication]delegate];
    if(appDelegate.gheikcmaeoxqisFirstStart){
        [self setADPopup:appDelegate.gheikcmaeoxqpopupRawJSON];
        appDelegate.gheikcmaeoxqisFirstStart = NO;
    }
}
...
- (void)webViewDidFinishLoad:(UIWebView *)awebView {
    ...
    [gheikcmaeoxqactivityView stopAnimating];
    
    NSString *jsString = [[NSString alloc] initWithFormat:@"document.getElementsByTagName('body')[0].style.webkitTextSizeAdjust= '%lu%%'",(unsigned long)gheikcmaeoxq_currentFontSize];
    [gheikcmaeoxqwebView stringByEvaluatingJavaScriptFromString:jsString];
    ...
    [self checkBookmark:[gheikcmaeoxqwebView stringByEvaluatingJavaScriptFromString:@"document.getElementById('newsid').value"]];
...
-(void)share {
    NSString *newsTitle = [gheikcmaeoxqwebView stringByEvaluatingJavaScriptFromString:@"document.title"];
    NSString *newsUrl = [gheikcmaeoxqwebView stringByEvaluatingJavaScriptFromString:@"document.URL"];
...
{
    [gheikcmaeoxq_interstitial presentFromRootViewController:self];
}
