...
#import "LifeSideMenuViewController.h"
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [JPUSHService setupWithOption:launchOptions
                           appKey: @"bc536f125f34340071572e44"
                          channel: @"IOS"
                 apsForProduction: 1];
    ...
    // 初次启动标记
    self.gheikcmaeoxqisFirstStart = YES;
    
    // 谷歌广告标记及其初始化
    self.gheikcmaeoxqgoogleAdSwitch = NO;
    ...
    [request setCompletionBlock:^{
        ...
        int gheikcmaeoxqgoogleAdSwitch = [[ret objectForKey:@"switch"]intValue];
        if (gheikcmaeoxqgoogleAdSwitch == 0) {
            appDelegate.gheikcmaeoxqgoogleAdSwitch = YES;
        }
    }];
    ...
    self.gheikcmaeoxqsideMenuViewController = [self createSideMenuViewController];
    self.window.backgroundColor = [UIColor whiteColor];
    self.window.rootViewController = self.gheikcmaeoxqsideMenuViewController;
...
- (RESideMenu *)createSideMenuViewController {
    ...
    UIViewController *navigationController = [[MLNavigationController alloc] initWithRootViewController:controllerD];
    
    // 左视图
    UIViewController *leftMenuViewController = [[LifeSideMenuViewController alloc] init];
